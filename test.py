...
def write_to_bin1(url_file, out_file, makevocab=False):
  """Reads the tokenized .story files corresponding to the urls listed in the url_file and writes them to a out_file."""
  print "Making bin file for URLs listed in %s..." % url_file
  url_list = read_text_file(url_file)
  # url_hashes = get_url_hashes(url_list)
  # story_fnames = [s+".story" for s in url_hashes]
  story_fnames = url_list
  num_stories = len(story_fnames)
...
if __name__ == '__main__':
  ...
  write_to_bin1(all_test_urls, os.path.join(finished_files_dir, "test.bin"))
  write_to_bin1(all_val_urls, os.path.join(finished_files_dir, "val.bin"))
  write_to_bin1(all_train_urls, os.path.join(finished_files_dir, "train.bin"), makevocab=True)
